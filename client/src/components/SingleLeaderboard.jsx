...
function SingleLeaderboard({ title, type, selectedTournamentId, setSelectedTournamentId, sortFn, filterFn, flipped = false }) {
  ...
  console.log('tournaments:', tournaments);  // Debug: what is tournaments?

  if (!Array.isArray(tournaments)) {
    console.warn("Warning: tournaments is not an array", tournaments);
    return <p>Error loading tournaments.</p>; // or some fallback UI
  }
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const res = await axios.get(`/api/leaderboard/${selectedTournamentId}`);
        ...
        if (handicapMode && showHandicappedOption && tournament?.type === "Stonehenge") {
          ...
          const responses = await Promise.all(
            dates.map(date => axios.get(`/api/handicaps/${date}`))
          );
          ...
          try {
            const hcapRes = await axios.get(`/api/handicaps/${endDate}`);
